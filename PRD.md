...
- [ ] **AVS/ZIP verification** (§5.3) — live-tested (2026-08-18) with AVS/ZIP turned on in the Valor portal ("Approve and Notify"), and the terminal did prompt for the ZIP, but no structured AVS match/no-match result came back in the response — just `AUTH_RSP_TEXT: "SERV UNAVAILABLE"` on an otherwise normal approval. Inconclusive from a demo/staging card alone (it likely has no real address on file to check against). Needs re-testing against a real card once in production to see what a genuine match/mismatch actually looks like in the response — the field is already captured and shown as "Auth Response" on the receipt and chargeback packet, so nothing further to build, just to verify.
